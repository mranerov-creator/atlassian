...
|----|------|---------|--------|
| **A0** | **CEO / Strategist** | **Strategy, pricing, positioning, pipeline** | ✅ Implemented |
| **A1** | **GTM & Partnerships** | **Pipeline generation, qualification, outreach** | ✅ Implemented |
| **A2** | **Solution Architect** | **TO-BE design, options, sizing, migration** | ✅ Implemented |
...
| **A5** | **Migration Factory** | **WP2/WP5 execution, wave planning, validation** | ✅ Implemented |
| **A6** | **Delivery PM/QA** | **QA, proposals, SoW, project management** | ✅ Implemented |
| **A7** | **Builder** | **Automations, integrations, Forge apps, dashboards** | ✅ Implemented |
...
### A0 CEO/Strategist - Strategic Leadership
...
```bash
bv a0 decide \
  --question "Should we expand to US market?" \
  --context "Growing US inquiries" \
  --output decision.md
```

### A2 Solution Architect - Architecture Design

#### Execute Full Architecture Design

```bash
bv a2 design \
  --client "Acme Corp" \
  --engagement "ARCH-ACME-001" \
...
bv a2 target \
  --client "Acme Corp" \
  --output target_state.md
```

#### Calculate Sizing & Licensing
...
### A5 Migration Factory - WP2/WP5 Migration
...
bv a5 wp2 \
  --client "Acme Corp" \
  --engagement "WP2-ACME-001" \
  --arch architecture.json \
  --output ./migration/
```

#### Execute WP5 Cloud-to-Cloud Migration
...
### A7 Builder - Implementation & Automation
...
### Architecture Design (A2)

```
initialize → assess_current_state → evaluate_options → design_target_state →
plan_migration → calculate_sizing → draft_document →
review → [approved] → finalize → END
      → [revise] → draft_document (loop)
```

**Architecture Deliverables:**
- Current state assessment
- Architecture options (3 options with scoring)
- Target state design
...
### Migration Planning (A5) - WP2/WP5

```
initialize → inventory → waves → runbooks → communications →
validation_plan → checklists → plan →
review → [approved] → finalize → END
      → [revise] → plan (loop)
```

**Migration Deliverables:**
- Migration inventory (projects, spaces, users, apps)
- Wave plan (pilot → main → cleanup)
- Migration runbooks (step-by-step procedures)
- Communication templates (pre/post migration)
- Validation plan (data integrity, UAT)
- Checklists (pre-wave, migration day, post-wave)
- Complete Migration Plan document

### Implementation (A7)

```
initialize → requirements → automations → scripts → integrations →
dashboards → package → review →
...
- Automation library (Jira/Confluence rules)
- Script library (ScriptRunner/Groovy)
- Integration catalog (webhooks, APIs)
- Dashboard & filter specifications
- Forge app specifications
...
- [x] A0 CEO/Strategist Agent
- [x] A1 GTM Agent
- [x] Lead Qualification Workflow
- [x] A2 Solution Architect Agent
...
- [x] A5 Migration Factory Agent
- [x] Migration Planning Workflow (WP2/WP5)
